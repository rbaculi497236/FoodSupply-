...
public sealed class WorkflowTests : IAsyncLifetime
{
    ...
    {
        var mysqlBase = Environment.GetEnvironmentVariable("FOODSUPPLY_TEST_MYSQL");
        if (!string.IsNullOrWhiteSpace(mysqlBase))
        {
            var builder = new MySqlConnector.MySqlConnectionStringBuilder(mysqlBase) {
                Database = "foodsupply_test_" + Guid.NewGuid().ToString("N")
            };
    ...
    private ApplicationDbContext NewContext() => new(mysqlConnection == null
    ...
    {
        // Only databases generated by this fixture can be removed.
        if (mysqlConnection != null && db != null &&
            System.Text.RegularExpressions.Regex.IsMatch(new MySqlConnector.MySqlConnectionStringBuilder(mysqlConnection).Database,
                "^foodsupply_test_[a-f0-9]{32}$"))
    ...
    [Fact]
    public async Task FilteredReportChartsUseAllMatchingRecords()
    {
        await Receive(100, days: 60);
        for (var i = 0; i < 12; i++)
        {
        ...
        var excluded = await Order(1);
        excluded.OrderDate = new DateTime(2026, 2, 15);
        var purchase = new Purchase { SupplierId = product.SupplierId, PurchaseOrderNumber = "PO-CHART", TotalAmount = 50,
            PurchaseDate = new DateTime(2026, 1, 15) };
        db.Add(purchase);
        db.Add(new Billing { SalesOrder = excluded, InvoiceNumber = "INV-CHART", TotalAmount = 10, AmountPaid = 4, Balance = 6, PaymentStatus = "Partial" });
        db.Add(new Delivery { SalesOrder = excluded, Status = "Pending" });
        await db.SaveChangesAsync();
        var reports = new ReportsController(db);
        await reports.Sales(new DateTime(2026, 1, 1), new DateTime(2026, 1, 31), 2);
        ReportChartViewModel Chart() => Assert.IsType<ReportChartViewModel>(reports.ViewData["Chart"]);
        Assert.Equal(120m, Assert.Single(Chart().Points).Value);
        await reports.Purchases(new DateTime(2026, 1, 1), new DateTime(2026, 1, 31));
        Assert.Equal(50m, Assert.Single(Chart().Points).Value);
        await reports.Inventory("In Stock");
        Assert.Equal(1m, Chart().Points.Sum(p => p.Value));
        await reports.Billing("Partial");
        Assert.Equal(new decimal[] { 4, 6 }, Chart().Points.Select(p => p.Value));
        await reports.Deliveries("Pending");
        Assert.Equal(1m, Assert.Single(Chart().Points).Value);
        await reports.Sales(new DateTime(2020, 1, 1), new DateTime(2020, 1, 31));
        Assert.Empty(Chart().Points);
    }

    [Fact]
    public async Task LoginAcceptsValidPasswordAndRejectsInvalidOrInactiveAccount()
    {
        var user = new User { FullName = "Test Admin", Username = "login-test", Email = "login@example.com", Role = "Admin" };
        user.PasswordHash = new Microsoft.AspNetCore.Identity.PasswordHasher<User>().HashPassword(user, "TestPassword123!");
        ...
        Assert.IsType<Microsoft.AspNetCore.Mvc.ViewResult>(await LoginController().Login(new LoginViewModel { Username = user.Username, Password = "wrong" }));
        Assert.Null(authentication.Principal);
        Assert.IsType<Microsoft.AspNetCore.Mvc.RedirectToActionResult>(await LoginController().Login(new LoginViewModel { Username = user.Username, Password = "TestPassword123!" }));
        ...
        Assert.IsType<Microsoft.AspNetCore.Mvc.ViewResult>(await LoginController().Login(new LoginViewModel { Username = user.Username, Password = "TestPassword123!" }));
        Assert.Null(authentication.Principal);
    }
    ...
    private sealed class TestAuthentication : Microsoft.AspNetCore.Authentication.IAuthenticationService
    {
        ...
        public Task SignInAsync(Microsoft.AspNetCore.Http.HttpContext context, string? scheme, System.Security.Claims.ClaimsPrincipal principal, Microsoft.AspNetCore.Authentication.AuthenticationProperties? properties) { Principal = principal; return Task.CompletedTask; }
        public Task SignOutAsync(Microsoft.AspNetCore.Http.HttpContext context, string? scheme, Microsoft.AspNetCore.Authentication.AuthenticationProperties? properties) => Task.CompletedTask;
        public Task ChallengeAsync(Microsoft.AspNetCore.Http.HttpContext context, string? scheme, Microsoft.AspNetCore.Authentication.AuthenticationProperties? properties) => Task.CompletedTask;
